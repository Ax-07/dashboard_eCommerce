...
const TopSellsList: React.FC<TopSellsListProps> = ({
    bestSeller,
    className,
    title = "Top Ventes",
    description = "Les produits les plus vendus",
}) => { 
    console.log("Top Sells", bestSeller);
    const products = PRODUCTS.find((p) => p.id === bestSeller[0]?.productId); console.log("Top Sells products", products);
    return (
                        ...
                        {bestSeller.map((item) => (
                            <li key={item.productId} className="flex items-center justify-between p-4 border rounded-md">
                                <div className="flex items-center gap-2">
                                    <img src={products?.media?.[0]?.url} alt={item.productName} className="w-10 h-10 object-cover rounded" />
