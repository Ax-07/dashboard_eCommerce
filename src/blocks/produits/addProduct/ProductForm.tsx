...
interface ProductFormProps {
  className?: string;
  productByID?: ProductInput;
}

export const ProductForm: React.FC<ProductFormProps> = ({className, productByID}) => {
  ...
  useEffect(() => {
    resetProduct(); // on reset le store au chargement du composant
    if (productByID) {
      setProduct(productByID); console.log(productByID);
  ...
  return (
    <Card className={cn("max-w-5xl mx-auto bg-transparent border-none", className)}>
      <CardHeader>
        <CardTitle className="text-2xl">{productByID ? "Modifier le produit" : "Créer un produit"}</CardTitle>
        <CardDescription>
