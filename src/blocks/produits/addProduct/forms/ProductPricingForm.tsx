// @/src/blocks/produits/forms/ProductPricingForm.tsx
"use client";
      ...
      replace([
        {
          name: "Prix d’achat",
          values: [
            { sku:"", name: "", quantity: 0, unit: "g", unitPrice: 0, totalPrice: 0 },
          ],
        },
        {
          name: "Prix de vente",
...
export const OptionValuesTable: React.FC<OptionValuesTableProps> = ({
  ...
  return (
    <table className="w-full table-fixed border-collapse mb-2">
      <colgroup>
        <col className="w-4/14" /> {/* 33% pour la 1re colonne */}
        <col className="w-3/14"/> {/* reste automatique */}
        <col />
        <col />
        <col />
        <col />
        <col className="w-1/14" /> {/* petite colonne pour le bouton */}
      </colgroup>
      <thead>
        <tr className="">
          {["Libellé (SKU)", "Nom", "Qté", "Unité", "Prix unitaire", "Prix total", ""].map(
            (h) => (
              <th
                key={h}
        ...
        {fields.map((val, vi) => (
          <tr key={val.id} className="even:bg-muted/50">
            <td className="">
              <SuggestField 
                form={form}
                controlPath={`options.${groupIndex}.values.${vi}.sku`}
                />
            </td>
            <td className="">
              <Input
                ...
                {...register(
                  `options.${groupIndex}.values.${vi}.sku` as const
                )}
              />
            </td>
            <td className="">
