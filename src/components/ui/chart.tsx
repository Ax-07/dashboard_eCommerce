                ...
                {formatter && item?.value !== undefined && item.name ? (
                  ...
                  <>
                    ...
                    >
                      <div className="grid gap-1.5">
                        {nestLabel ? tooltipLabel : null}
                        <span className={cn("text-muted-foreground")}>{itemConfig?.label || item.name}</span>
                      </div>
                      {item.value && (
...
export const CustomLegend: React.FC<CustomLegendProps> = ({
  ...
  return (
    <div className={cn("flex items-center justify-center gap-4", verticalAlign === "top" ? "pb-3" : "pt-3", className)}>
      {payload.map((item) => {
        // Déterminer la clé brute (rawKey)
        let rawKey: string;
        if ("dataKey" in item && item.dataKey != null) {
          rawKey = String(item.dataKey);
        } else {
          const valueLabel = String(item.value);
          rawKey = Object.keys(config).find((k) => String(config[k].label) === valueLabel) || valueLabel;
        }

        const isHidden = hiddenKeys?.includes(rawKey);
        return (
          <div
            key={rawKey}
            onClick={() => onToggleKey &&  onToggleKey(rawKey)}
            className={cn(
              "flex items-center gap-1.5 [&>svg]:h-3 [&>svg]:w-3 [&>svg]:text-muted-foreground",
              onToggleKey && "cursor-pointer",
              isHidden && "line-through opacity-40"
            )}
          >
            <span className="h-2 w-2 shrink-0 rounded-[2px]" style={{ backgroundColor: item.color }} />
            {/* Afficher le label configuré ou la valeur brute */}
            {config[rawKey]?.label ?? item.value}
          </div>
        );
      })}
    </div>
